...
def menu_edit(request):
    ...
    if request.method == "GET":
        ...
        menu_editform_obj = forms.MenuForm(request.POST)
        if menu_editform_obj.is_valid():
            models.Menu.objects.filter(id=id).update(request.POST)
        else:
            ret["status"]=False
            ret["errors"]=menu_editform_obj.errors.as_json()

    ret["data"]=menu_editform_obj
    return HttpResponse(json.dumps(ret))
...
def menu2_list(request):
    ...
    menu2_objs = models.Menu2.objects.all().order_by("-id")
    page_objs = page(request, menu2_objs, 20)
